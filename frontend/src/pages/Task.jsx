...
const Task = () => {
  ...
  useEffect(() => {
    if (mode === "update") {
      ...
      fetchData(config, { showSuccessToast: false }).then((data) => {
        setTask(data.task);
        setFormData({ description: data.task.description, deadlineDate: data.task.deadlineDate });
      });
    }
  ...
  const handleSubmit = e => {
    ...
    setFormErrors({});
    // Additional validation for deadline field
    if (formData.deadlineDate && new Date(formData.deadlineDate) < new Date()) {
      errors.push({ field: "deadline", err: "Deadline must be in the future" });
    }
